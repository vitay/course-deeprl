...
* We only have access to **samples** of $r(a)$ by taking the action at time $t$ (a **trial**, **play** or **step**).
...
![](img/bandit-nonstationary2.png){width=50%}
...
* $\text{target} - \text{old estimate}$ is the **prediction error** between the target and the estimate.
...
![](img/exploration_vs_exploitation.png){width=45%}
...
![](img/narmedbandit2.jpg){width=60%}
...
* The preferences can be used to select the action using the soft-max method just as the Q-values (without temperature):
...
* One can even go further than reinforcement comparison:

    * Instead of only increasing the preference for the executed action if it brings more usual than rewards, we could also decrease the preference for the other actions.
...
![Source: <https://medium.com/emergent-future/simple-reinforcement-learning-with-tensorflow-part-1-5-contextual-bandits-bff01d1aad9c>](img/bandit-hierarchy.png)
...
![Source: <https://aws.amazon.com/blogs/machine-learning/power-contextual-bandits-using-continual-learning-with-amazon-sagemaker-rl/>](img/contextualbandit.gif)
