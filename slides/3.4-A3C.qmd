...
$$
\begin{aligned}
A^n_t &= R_t^n - V_\varphi (s_t)\\
      &= \sum_{k=0}^{n-1} \gamma^{k} \, r_{t+k+1} + \gamma^n \,  V_\varphi(s_{t+n}) - V_\varphi (s_t)\\
\end{aligned}
$$
...
* The **actor** $\pi_\theta(s, a)$ uses the PG theorem to learn the policy:
